...
//  Adapted slightly for NumWorks (optional confirmation prompt).
//
...
enum AppMover {
    ...
    /// Prompts (unless `prompt` is false) and moves the running app into
    /// Applications when it is not already installed there.
    static func moveIfNecessary(prompt: Bool = true) {
        ...
        let needDestAuth = fm.fileExists(atPath: destinationUrl.path)
            && !fm.isWritableFile(atPath: destinationUrl.path)
        ...
        if prompt {
            ...
            if needAuth {
                alert.informativeText.append(
                    " You need to authenticate with your administrator password to complete this step.")
            }
    ...
    static func authorizedInstall(from sourceURL: URL, to destinationURL: URL)
        -> (cancelled: Bool, success: Bool)
    {
        ...
        return sourceURL.withUnsafeFileSystemRepresentation({ sourcePath -> (cancelled: Bool, success: Bool) in
            return destinationURL.withUnsafeFileSystemRepresentation({ destinationPath -> (cancelled: Bool, success: Bool) in
                ...
                let copyCommand =
                    "cp -pR '\(String(cString: sourcePath))' '\(String(cString: destinationPath))'"
                guard let script = NSAppleScript(source:
                    "do shell script \"\(deleteCommand) && \(copyCommand)\" with administrator privileges")
                else {
                    return (false, false)
                }
    ...
    static func relaunch(at path: String, completionCallback: @escaping () -> Void) {
